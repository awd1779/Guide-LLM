...
def embed_images(folder_path, collection):
    ...
    for image_path in tqdm(image_files, desc="Embedding images"):
        ...
        # Build the compound class string.
        # Here, we assume the image is stored directly in a folder (e.g. "First_Floor")
        # and the image file name (without extension) is the second component (e.g. "HallwayA_north").
        floor_name = os.path.basename(os.path.dirname(image_path))
        image_name = os.path.splitext(os.path.basename(image_path))[0]
        class_name = floor_name + "|" + image_name
        
        image_id = image_name  # Using the image file name (without extension) as the id
        
        # Add the embedding and metadata to ChromaDB
        collection.add(
...
if __name__ == "__main__":
    ...
    image_folder_path = "/home/ubuntu/catkin_ws/src/scripts/vdb/area4_images"  # Adjust if needed
